...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR args, int iCmdShow)
{
	...
	app[WM_CREATE] =
		...
		app.create_menu(
			...
			.add_menu_item(MF_STRING,
				...
				[&](HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) -> DWORD {
					logp(sys::e_trace, "");
					logp(sys::e_trace, "** Get windows menu click. HWND " << hwnd);
					...
					if (cur_desktop) {
						...
						cur_desktop = virt.get_desktop_id_from_current_session();
						if (cur_desktop) {
							logp(sys::e_trace, "Current desktop: '" << win::guid_to_string(&cur_desktop.value())
								<< "'");
						}
						else {
							logp(sys::e_warning, "There is no current desktop.");
						}
					}
					win::virt_desktop_t::opt_guid_vect vect = virt.get_virtual_desktop_ids_from_registry(win::virt_desktop_t::get_virtual_desktops_reg_key());
					if (vect) {
						for (auto id : vect.value()) {
							logp(sys::e_trace, "Desktop id: '" << win::guid_to_string(&id) << "'");
						}
					}
					else {
						logp(sys::e_warning, "Can't get virtual desktop ids.");
					}
					win::virt_desktop_t::opt_guid guid = virt.get_desktop_id_for_window(hwnd);
					if (guid) {
						logp(sys::e_trace, "Desktop for window: '" << win::guid_to_string(&guid.value()) << "'");
					}
					return 0;
				})
				.add_menu_item(MF_STRING,
	...
	*/
	...
	app[WM_TRAYICON] =
		...
		logp(sys::e_trace, "Clicked option " << clicked << ".");
